 ...
 * prohibited.
 *
 * Description: CIR Fulfillment chart wrapper component (CHART-04)
 */
...
import {useCirFulfillmentMock} from './useCirFulfillmentMock';
...
const CirFulfillment: React.FC<ChartWrapperProps> = ({
  registerChart,
  unregisterChart,
  onSetExtremes,
  eventMarkers
}) => {
  const {series, isLoading, isEmpty, error} = useCirFulfillmentMock();
  ...
  if (error) {
    return (
      <EmptyStripContainer>
        <Text kind="label-md" bold style={{marginBottom: 4}}>CIR Fulfillment</Text>
