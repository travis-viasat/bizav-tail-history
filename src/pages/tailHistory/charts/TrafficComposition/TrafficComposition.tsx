 ...
 * prohibited.
 *
 * Description: Traffic Composition stacked area chart wrapper component (CHART-05)
 */
...
import {useTrafficCompositionMock} from './useTrafficCompositionMock';
...
const TrafficComposition: React.FC<ChartWrapperProps> = ({
  registerChart,
  unregisterChart,
  onSetExtremes,
  eventMarkers
}) => {
  const {series, isLoading, isEmpty, error} = useTrafficCompositionMock();
  ...
  if (error) {
    return (
      <EmptyStripContainer>
        <Text kind="label-md" bold style={{marginBottom: 4}}>Traffic Composition</Text>
