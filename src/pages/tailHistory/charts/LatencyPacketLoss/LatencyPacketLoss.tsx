 ...
 * prohibited.
 *
 * Description: Latency & Packet Loss chart wrapper component (CHART-06)
 */
...
import {useLatencyPacketLossMock} from './useLatencyPacketLossMock';
...
const LatencyPacketLoss: React.FC<ChartWrapperProps> = ({
  registerChart,
  unregisterChart,
  onSetExtremes,
  eventMarkers
}) => {
  const {series, isLoading, isEmpty, error} = useLatencyPacketLossMock();
  ...
  if (error) {
    return (
      <EmptyStripContainer>
        <Text kind="label-md" bold style={{marginBottom: 4}}>Latency &amp; Packet Loss</Text>
  ...
  return (
    <ChartStrip
      ...
      height={CHART_HEIGHT}
      legendEnabled={true}
      yAxisOptions={{title: {text: 'ms', align: 'high'}}}
      yAxisAdditional={[{title: {text: '%', align: 'high'}, opposite: true, min: 0, max: 100}]}
      eventMarkers={eventMarkers}
    />
  );
};

export default LatencyPacketLoss;
