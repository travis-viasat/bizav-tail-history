 ...
 * prohibited.
 *
 * Description: Service Availability chart wrapper component (CHART-03)
 */
...
import {useServiceAvailabilityMock} from './useServiceAvailabilityMock';
...
const ServiceAvailability: React.FC<ChartWrapperProps> = ({
  registerChart,
  unregisterChart,
  onSetExtremes,
  eventMarkers
}) => {
  const {series, isLoading, isEmpty, error} = useServiceAvailabilityMock();
  ...
  if (error) {
    return (
      <EmptyStripContainer>
        <Text kind="label-md" bold style={{marginBottom: 4}}>Service Availability &amp; CIR Satisfaction</Text>
  ...
  return (
    <ChartStrip
      ...
      height={CHART_HEIGHT}
      legendEnabled={true}
      yAxisOptions={{title: {text: undefined}, min: 0, max: 100}}
