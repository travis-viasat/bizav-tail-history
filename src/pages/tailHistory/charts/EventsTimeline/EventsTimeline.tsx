 ...
 * prohibited.
 *
 * Description: Events Timeline chart wrapper component (CHART-01)
 */
...
import {useEventsTimelineMock} from './useEventsTimelineMock';
...
const EventsTimeline: React.FC<ChartWrapperProps> = ({
  registerChart,
  unregisterChart,
  onSetExtremes
}) => {
  const {series, isLoading, isEmpty, error} = useEventsTimelineMock();
  ...
  if (error) {
    return (
      <EmptyStripContainer>
        <Text kind="label-md" bold style={{marginBottom: 4}}>Events Timeline</Text>
